...
def solve_string_conditions(exprs, var_names, number_of_solutions=3):
    ...
    try:
        ...
        logger.warning(
            f"Ошибка при парсинге выражения: {e}\nВыражение: {expr_str_prepared}"
        )
        return None
...
def solve_conditions(
    ...
    try:
        if " or " in expr_str:
            parts = [
                eval(part, context)
                for part in expand_expressions(expr_str, " or ", orig_exprs)
            ]
            z3_expr = Or(*parts)
        elif " and " in expr_str:
            parts = [
                eval(part, context)
                for part in expand_expressions(expr_str, " and ", orig_exprs)
            ]
            z3_expr = And(*parts)
        else:
            z3_expr = eval(expr_str, context)
    except Exception as e:
        logger.warning(f"Ошибка при парсинге выражения: {e}\nВыражение: {expr_str}")
        return None
